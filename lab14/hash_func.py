...
def streebog256(message: bytes) -> bytes:
    # Начальный вектор для 256-битной версии:
    hash = bytearray([0x01] * 64)
    streebog_core(message, hash)
    result = hash[-32:]
    # Для 256-битной версии берем последние 32 байта
...
def sha256(message):
    hex_digest = sha_generic(message, 32, 64, SHA256_INITIAL_HASH, SHA256_K, 64)
    return hex_digest

def sha512(message):
    hex_digest = sha_generic(message, 64, 128, SHA512_INITIAL_HASH, SHA512_K, 80)
    return hex_digest
